...
- `src/main/java/.../SecurityConfig.java`: configures reactive filter chain
- `src/main/java/.../PermissionResolver.java`: maps (role, status) to permissions
```

### Tag Rules
- Max 10 tags per document
- All tags start with `#`
- Use `_` instead of spaces: `#spring_security` not `#spring security`
...
If the project maintains design specs (e.g., in `docs/superpowers/specs/` or similar), the memory bank and specs serve different purposes:
...
If using the superpowers plugin: completed specs and plans are renamed from `{name}.md` to `{name}-done.md`. When referencing specs, check for the `-done.md` suffix to determine completion status.
...
### Search Strategies
- **By tag:** Search frontmatter for tags like `#spring_security`, `#polls`
...
**After any planning session** — brainstorming, plan-mode, or writing-plans skill (if superpowers plugin is available) — when a design spec or plan is written and committed:
